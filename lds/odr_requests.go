...
func LdsRequest(req light.OdrRequest) LdsOdrRequest {
	switch r := req.(type) {
...
// Request sends an ODR request to the LES network (implementation of LdsOdrRequest)
...
// to the request (implementation of LdsOdrRequest)
...
// ODR request type for state/storage trie entries, see LdsOdrRequest interface
...
// ODR request type for node data (used for retrieving contract code), see LdsOdrRequest interface
...
// ODR request type for requesting headers by Canonical Hash Trie, see LdsOdrRequest interface
